...
struct my_item {
  ...
  bool stealth = false; // Hero can sneak past monsters (but cannot loot items while sneaking).
  void print() const {
    std::cout << "hp: " << hp << " off: " << off << " def: " << def << " soff: " << stacking_off << " sdef: " << stacking_def;
    std::cout << " 1st_at" << first_attack << " stealth " << stealth << std::endl;
  }
...
bool check_if_monster_defeatable(Player human, Monster monsteros) {
  ...
  if(player_first) {
    ...
    if (result == A_WINS) return true;
    else return false;
  } else {
    auto result = simulate_combat(monsteros, human.make_monster());
    if(result == B_WINS) return true;
    else return false;
  }
}
struct Node_State {
  ...
  void print() const {
    ...
    std::cout << "RoomId: " << this->node << " has treasure: " << (has_treasure ? "1": "0") << std::endl;
    for (int i = 0; i < 3; i++) {
...
std::vector<Action> create_action_vector(const std::map<Node_State,std::pair<Node_State,Action>> & map, const Node_State & last, const Node_State & first) {
...
std::vector<Action> find_shortest_path(
  ...
  while(to_visit.size() > 0) {
    ...
    if(std::find(visited.begin(), visited.end(), current_node_state) != visited.end()) {
      std::cout << "uz som tu bol" << std::endl;
      to_visit.pop();
      continue;
    }
    to_visit.pop();
    visited.emplace(current_node_state);
    ...
    if (!rooms[current_node_state.node].monster.has_value() || check_if_monster_defeatable(human, rooms[current_node_state.node].monster.value())) { //ci ho mozem fightit
      ...
      if (current_node_state.has_treasure == true && std::find(entrances.begin(), entrances.end(), current_node_state.node) != entrances.end()) {
      ...
      if (human.inventory[0] != my_item()) {
        weapons_available[0].push_back(human.inventory[0]);
      }
      if (human.inventory[1] != my_item()) {
        weapons_available[1].push_back(human.inventory[1]);
      }
      if (human.inventory[2] != my_item()) {
        weapons_available[2].push_back(human.inventory[2]);
      }

      for (size_t i = 0; i < rooms[current_node_state.node].items.size(); i++) {
        my_item current_item(rooms[current_node_state.node].items[i]);
        current_item.index = i;
        if(current_item.type == 0) weapons_available[0].push_back(current_item);
        else if (current_item.type == 1) weapons_available[1].push_back(current_item);
        else if (current_item.type == 2) weapons_available[2].push_back(current_item);
      }
      std::cout << "koniec vyrabania vector itemov" << std::endl;
      //itemy spracovane idem spravit vsetky mozne kombinacie
      Node_State working_state;
      working_state.inventory = human.inventory;
      working_state.node = current_node_state.node;
      working_state.has_treasure = current_node_state.has_treasure;
      Node_State previous_state;
      previous_state.inventory = human.inventory;
      previous_state.node = current_node_state.node;
      previous_state.has_treasure = current_node_state.has_treasure;
      ItemId id;
      ...
      for (size_t weapon_index = 0; weapon_index < weapons_available[0].size(); weapon_index++) { //weapon
        if ((weapon_index == 0) && (working_state.inventory[0] != my_item())) { //drop action
          std::cout << "dropujem weapon" << std::endl;
        ...
        } else if (weapon_index != 0) { //pickup action
          std::cout << "pickujem weapon" << std::endl;
          working_state.inventory[0] = weapons_available[0][weapon_index];
          id = working_state.inventory[0].index;
          previous_action[working_state] = std::make_pair(previous_state, Pickup(id));
          previous_state = working_state;
        }
        for (size_t armor_index = 0; armor_index < weapons_available[1].size(); armor_index++) { //armor
          if ((armor_index == 0) && (working_state.inventory[1] != my_item())) { //drop action
            std::cout << "dropujem armor" << std::endl;
            working_state.inventory[1] = my_item();
            previous_action[working_state] = std::make_pair(previous_state, Drop(Item::Armor));
            previous_state = working_state;
          } else if (armor_index != 0) { //pickup action
            std::cout << "pickujem armor" << std::endl;
            working_state.inventory[1] = weapons_available[1][armor_index];
            id = working_state.inventory[1].index;
            previous_action[working_state] = std::make_pair(previous_state, Pickup(id));
            previous_state = working_state;
          }
          for (size_t duck_index = 0; duck_index < weapons_available[2].size(); duck_index++) { // duck
            if ((duck_index == 0) && (working_state.inventory[2] != my_item())) { //drop action
              std::cout << "dropujem duck" << std::endl;
              working_state.inventory[2] = my_item();
              previous_action[working_state] = std::make_pair(previous_state, Drop(Item::RubberDuck));
              previous_state = working_state;
            } else if (duck_index != 0) { //pickup action
              std::cout << "pickujem duck" << std::endl;
              working_state.inventory[2] = weapons_available[2][duck_index];
              id = working_state.inventory[2].index;
              previous_action[working_state] = std::make_pair(previous_state, Pickup(id));
              previous_state = working_state;
            }
      ...
      //tu vlozim vsetkych neighbours s terajsim inventarom alebo dropnutym inventarom
      ...
      for (auto i : rooms[current_node_state.node].neighbors) {
              ...
              to_visit.push(working_state);
              //dropnem len 1 item
              for (int slot = 0; i < 3; slot++) {
                working_state.inventory[slot] = my_item();
                to_visit.push(working_state);
                working_state.inventory[slot] = current_node_state.inventory[slot];
              }
              //dropnem 2 itemy
              working_state.inventory[0] = my_item();
              working_state.inventory[1] = my_item();
              to_visit.push(working_state);
              working_state.inventory[0] = current_node_state.inventory[0];
              working_state.inventory[1] = current_node_state.inventory[1];

              working_state.inventory[0] = my_item();
              working_state.inventory[2] = my_item();
              to_visit.push(working_state);
              working_state.inventory[0] = current_node_state.inventory[0];
              working_state.inventory[2] = current_node_state.inventory[2];
  ...
  if (found_path) {
    ...
    result = create_action_vector(previous_action, current_node_state, first_room);
  } 
