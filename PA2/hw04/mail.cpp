...
public:
  ...
  if (x.subject().has_value()) os << ", subject: " << x.subject().value();
  return os;
  }
};
...
namespace MysteriousNamespace
{
  ...
  bool compare(const CMail &a, const CMail &b)
  {
  ...
  public:
    int parseLog(std::istream &in)
    {
      ...
      for (std::string line; std::getline(in, line, '\n');)
      {
        ...
        else if (line.find("to=") != std::string::npos)
        {
          ...
          logs[id].t = newstamp;
          CMail newmail(logs[id].t, logs[id].from, logs[id].to, logs[id].subject);
    ...
    std::list<CMail> listMail(const CTimeStamp &from,
      ...
      std::list<CMail> result(iter_from, iter_to);         
      return result;
    }
