...
class CPolynomial {
 public:
  ...
  CPolynomial operator*(const CPolynomial& other) const {
    ...
    other.remove_zeros();
    int total_length =
        this->coefficients.size() + other.coefficients.size() - 1;
  ...
  bool operator==(const CPolynomial& other) const {
    ...
    for (size_t i = 0; i < this->coefficients.size() - 1; i++) {
      ...
      if (!(abs(this->coefficients[i] - other.coefficients[i]) < 0.0001))
        return false;
    }
    return true;
  }
 ...
 private:
  std::ostream& print(std::ostream& os) const {
    ...
    if (this->coefficients[0] > 0)
      os << " + " << this->coefficients[0];
    else if (this->coefficients[0] < 0)
      os << " - " << this->coefficients[0] * -1.0;
...
#ifndef __PROGTEST__
bool smallDiff(double a, double b) { return abs(a - b) < 0.0001; }
